...
fn worker_loop(
    ...
    tracing::info!("transcribe-rs parakeet model loaded");

    while let Ok((samples, reply)) = rx.recv() {
        let result = if samples.0.len() < 1600 {
            Ok(String::new())
        } else {
            transcribe_inner(&mut model, &samples.0)
        };
...
fn load_model(model_dir: &Path, engine: ModelEngine) -> Result<ParakeetModel> {
    match engine {
        ModelEngine::Parakeet => ParakeetModel::load(model_dir, &Quantization::Int8)
            .map_err(|e| anyhow!("failed to load parakeet model: {e}")),
        _ => Err(anyhow!("only Parakeet model is supported")),
    }
}

fn transcribe_inner(model: &mut ParakeetModel, audio: &[f32]) -> Result<String> {
